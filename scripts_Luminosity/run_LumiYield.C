...
void run_LumiYield(Int_t RunNumber = 0, Int_t MaxEvent = 0, Double_t threshold_cut = 5, Int_t pscal = 1)
{
  ...
  fstream REPORT_file;
  REPORT_file.open (Form("~/Analysis/hallc_replay_kaonlt/REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_production_%i_%i.report",RunNumber,MaxEvent));
  Int_t line_num = 0;
  string line;
  TString line_PS1;
  TString line_PS3;
  ...
  TChain ch("T");
  ch.Add(Form("~/Analysis/hallc_replay_kaonlt/ROOTfiles/KaonLT_coin_replay_production_%i_%i.root",RunNumber,MaxEvent));
  ...
  TChain sc("TSH");
  sc.Add(Form("~/Analysis/hallc_replay_kaonlt/ROOTfiles/KaonLT_coin_replay_production_%i_%i.root",RunNumber,MaxEvent));
