...
# Section for kaon replay script
...
if [ ! -f "$UTILPATH/ROOTfiles/Analysis/KaonLT/Kaon_coin_replay_production_${RUNNUMBER}_${MAXEVENTS}.root" ]; then
    if [[ "${HOST}" != *"ifarm"* ]]; then
	if [[ "${HOST}" == *"cdaq"* ]]; then
	    eval "$REPLAYPATH/hcana -l -q \"UTIL_KAON/scripts/replay/KaonLT/replay_production_coin.C($RUNNUMBER,$MAXEVENTS)\"" | tee $UTILPATH/REPORT_OUTPUT/Analysis/KaonLT/Kaon_output_coin_production_Summary_${RUNNUMBER}_${MAXEVENTS}.report
	else	
	    eval "$REPLAYPATH/hcana -l -q \"UTIL_KAON/scripts/replay/KaonLT/replay_production_coin.C($RUNNUMBER,$MAXEVENTS)\"" 
	fi
