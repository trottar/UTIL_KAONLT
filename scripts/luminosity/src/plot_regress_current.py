...
# Description:
# ================================================================
# Time-stamp: "2023-08-30 22:29:41 trottar"
# ================================================================
#
...
def plot_regress(settingList, momentumList, spec):
    ...
    with PdfPages(SCRIPTPATH+'/luminosity/OUTPUTS/plots/{}_regression_current_{}.pdf'.format(spec.lower(), target)) as pdf:
        ...
        for i, s in enumerate(settingList):
            ...
            eff_boil_list.append(eff_boil.values)  # Append eff_boil value to the list
        ...
        aver_eff_boil = np.mean(eff_boil_list)
        plt.plot([min(dataDict[s]['run number']), max(dataDict[s]['run number'])], [aver_eff_boil,aver_eff_boil], color='r', linestyle='dotted', label='Average Eff Boil: {}'.format(aver_eff_boil))
