...
# Description:
# ================================================================
# Time-stamp: "2023-08-30 22:20:20 trottar"
# ================================================================
#
...
def plot_regress(settingList, momentumList, spec):
    ...
    with PdfPages(SCRIPTPATH+'/luminosity/OUTPUTS/plots/{}_regression_current_{}.pdf'.format(spec.lower(), target)) as pdf:
        ...
        plt.plot([min(dataDict[s]['run number']), max(dataDict[s]['run number'])], [aver_eff_boil]*len(dataDict[s]['run number']), color='r', linestyle='dotted', label='Average Eff Boil: {}'.format(aver_eff_boil))
