...
if [ ! -f "$REPLAYPATH/UTIL_KAONLT/ROOTfiles/Analysis/HeeP/Pion_${spec}_replay_production_${RUNNUMBER}_${MAXEVENTS}.root" ]; then
    if [[ "${HOSTNAME}" != *"ifarm"* ]]; then
	if [[ "${HOSTNAME}" == *"cdaq"* ]]; then
	    eval "$REPLAYPATH/hcana -l -q \"UTIL_KAONLT/scripts/replay/replay_${spec}_heep.C($RUNNUMBER,$MAXEVENTS)\""| tee $REPLAYPATH/UTIL_KAONLT/REPORT_OUTPUT/Analysis/HeeP/Pion_output_${spec}_production_Summary_${RUNNUMBER}_${MAXEVENTS}.report
	else	
	    eval "$REPLAYPATH/hcana -l -q \"UTIL_KAONLT/scripts/replay/replay_${spec}_heep.C($RUNNUMBER,$MAXEVENTS)\"" 
	fi
