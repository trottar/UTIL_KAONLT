void replay_coin_heep (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "UTIL_KAONLT/ROOTfiles/Analysis/HeeP/Pion_coin_replay_production_%d_%d.root";
  ...
  // Define cuts file
  analyzer->SetCutFile("UTIL_KAONLT/config/DEF-files/Online_Coin_Production_Cuts.def");  // optional
  ...
  analyzer->PrintReport("UTIL_KAONLT/config/TEMPLATES/COIN/Online_Coin_Production.template", Form("UTIL_KAONLT/REPORT_OUTPUT/Analysis/HeeP/Pion_replay_coin_production_%d_%d.report", RunNumber, MaxEvent)); // optional}
}
