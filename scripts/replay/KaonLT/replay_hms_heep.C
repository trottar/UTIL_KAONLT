void replay_hms_heep (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "UTIL_KAONLT/ROOTfiles/Analysis/HeeP/Kaon_hms_replay_production_%d_%d.root";
  ...
  analyzer->PrintReport("UTIL_KAONLT/config/TEMPLATES/Online_HeePSing.template", Form("UTIL_KAONLT/REPORT_OUTPUT/Analysis/HeeP/Kaon_replay_hms_production_%d_%d.report", RunNumber, MaxEvent)); // optional}
}
