void replay_production_shms_coin (Int_t RunNumber = 0, Int_t MaxEvent = 0, Int_t FirstEvent = 1) {
  ...
  const char* ROOTFileNamePattern = "UTIL_KAONLT/ROOTfiles/Analysis/KaonLT/KaonLT_shms_coin_replay_production_%d_%d.root";
  ...
  analyzer->SetSummaryFile(Form("UTIL_KAONLT/REPORT_OUTPUT/Analysis/KaonLT/summary_coin_production_%d_%d.report", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("UTIL_KAONLT/TEMPLATES/SHMS/pstackana_production.template",
  			Form("UTIL_KAONLT/REPORT_OUTPUT/Analysis/KaonLT/KaonLT_replay_shms_coin_production_%d_%d.report", RunNumber, MaxEvent));  // optional
