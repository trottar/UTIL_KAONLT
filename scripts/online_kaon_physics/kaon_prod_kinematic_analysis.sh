...
if [ $TestingVar == 1 ]; then
    ...
    if [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/${runNum}_-1_Output_Data.root" ]; then
        python3 ${UTILPATH}/scripts/online_pion_physics/PlotKaonPhysics_sw.py Analysed_Data ${runNum} -1
    elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/${runNum}_-1_sw_Pion_Analysis_Distributions.pdf" ]; then
	python3 ${UTILPATH}/scripts/online_pion_physics/PlotKaonPhysics_sw.py Analysed_Data ${runNum} -1
    else echo "Pion plots already found in - ${UTILPATH}/OUTPUT/Analysis/KaonLT/${runNum}_-1_Output_Data.root and .pdf - Plotting macro skipped"
    fi
fi
