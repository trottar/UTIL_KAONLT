...
void hms_cer_efficiency::Terminate()
{
  ...
  cID->Update();
  cID->Update();
  TLine *LambdaMass_Full_noID = new TLine(1.1156,gPad->GetUymin(),1.1156,gPad->GetUymax()); 
  LambdaMass_Full_noID->SetLineColor(kBlack); LambdaMass_Full_noID->SetLineWidth(3);
  LambdaMass_Full_noID->SetLineStyle(2);
  LambdaMass_Full_noID->Draw();
  ...
  LambdaMass_Full->SetLineStyle(2);
