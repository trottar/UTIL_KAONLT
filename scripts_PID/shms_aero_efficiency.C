#define shms_aero_efficiency_cxx
// The class definition in shms_aero_efficiency.h has been generated automatically
...
//
// root> T->Process("shms_aero_efficiency.C")
// root> T->Process("shms_aero_efficiency.C","some options")
// root> T->Process("shms_aero_efficiency.C+")
//


#include "shms_aero_efficiency.h"
#include <TStyle.h>
#include <TCanvas.h>
#include <TLine.h>
#include <TMath.h>
...
void shms_aero_efficiency::Begin(TTree * /*tree*/)
{
...
void shms_aero_efficiency::SlaveBegin(TTree * /*tree*/)
{
  ...
  h2ROC1_Coin_Beta_noID_electron = new TH2F("ROC1_Coin_Beta_noCut_electron","Electron Coincident Time vs Mass (GeV/c^{2}) for ROC1 (w/out Aerogel cuts);Time (ns);Mass (GeV/c^{2})",800,-5,5,200,0.0,2.0);
  h2ROC1_Coin_Beta_electron = new TH2F("ROC1_Coin_Beta_electron","Electron Coincident Time vs Mass (GeV/c^{2}) for ROC1 (w/ Aerogel cuts);Time (ns);Mass (GeV/c^{2})",800,-5,5,200,0.0,2.0);
...
Bool_t shms_aero_efficiency::Process(Long64_t entry)
{
  ...
  if(P_cal_etotnorm[0] > 0.7 || P_hgcer_npeSum[0] > 6.0){
    h2ROC1_Coin_Beta_noID_electron->Fill((CTime_eKCoinTime_ROC1[0] - 47.5),sqrt(pow(emiss[0],2)-pow(pmiss[0],2)));
  }

  if(P_aero_npeSum[0] > 15.0){
  // if(P_cal_etotnorm[0] > 0.7 || P_hgcer_npeSum[0] > 6.0 || P_aero_npeSum[0] > 1.5){
...
void shms_aero_efficiency::SlaveTerminate()
{
...
void shms_aero_efficiency::Terminate()
{
